...
export default async function(db: ioredis.Redis, config: Config) {
    ...
    let cursor = 0;
    let done_count = 0;
    let total_count = await db.hlen(speedrun_db.locs.genres);

    let indexer = load_indexer(config, 'genres');

    do {
        console.log('Rescore Genres:', done_count, '/', total_count);

        let res = await db.hscan(speedrun_db.locs.genres, cursor);

        cursor = res[0];

        // cursor returns both keys and values. Iterate by values.
        for(let i = 1;i < res[1].length;i += 2) {
            await speedrun_db.rescore_genre(db, indexer, JSON.parse(res[1][i]));
        }
